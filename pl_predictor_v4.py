"""
╔══════════════════════════════════════════════════════════════════════╗
║          PREMIER LEAGUE SEASON PREDICTOR  ·  v4                     ║
║          Machine Learning · Probabilistic Forecasting               ║
╚══════════════════════════════════════════════════════════════════════╝

Built by an analytics consultant to demonstrate end-to-end ML pipeline
design: data engineering, temporal feature construction, walk-forward
model validation, Monte Carlo simulation, and production-grade output.

────────────────────────────────────────────────────────────────────────
WHAT'S NEW IN v4
────────────────────────────────────────────────────────────────────────
  ▸ Data gap awareness
    Detects the cutoff date of your CSV data vs. today. Flags any
    results you're missing and calculates prediction staleness.

  ▸ Real fixture injection
    GW32 fixture list embedded directly — no more "unknown pairs"
    in the remaining-fixture calculation. Predictions are tied to
    real upcoming matches, not combinatorial guesses.

  ▸ Progressive form blending (v3 feature, refined)
    SeasonWeight = GamesPlayed / 38
    At GW5  → 13% this season, 87% historical baseline
    At GW20 → 53% this season, 47% historical
    At GW31 → 82% this season, 18% historical
    Model adapts its reliance on current-season evidence over time.

  ▸ Current-season table as a model input
    HomePts_Season / AwayPts_Season / PtsDiff_Season fed directly
    into the gradient booster. Model literally sees the live table.

  ▸ ELO with seasonal decay (10% regression to mean each August)
    Prevents stale dominance — a team's 2022 peak doesn't make them
    look like a 2026 contender.

  ▸ Odds-era-only training
    Seasons pre-2019/20 had no AvgH/D/A data — training on them
    poisoned the model with flat-prior noise. Restricted to 6
    seasons of clean odds data.

  ▸ HTML dashboard auto-generated
    Self-contained dark-theme dashboard exported on every run.
    No server needed — open pl_dashboard_v4.html in any browser.
    Designed for GitHub README screenshots and LinkedIn posts.

────────────────────────────────────────────────────────────────────────
USAGE
────────────────────────────────────────────────────────────────────────
  Google Colab:
    1. Uncomment the 3 Colab lines in load_data()
    2. Upload your E0*.csv season files
    3. Run all cells — dashboard auto-exports

  Local:
    python pl_predictor_v4.py
    (CSV files must be in the same directory)

  Inject real bookmaker odds into predict_match() for best accuracy:
    predict_match(model, df, 'Arsenal', 'Man City', elo_dict,
                  standings, home_odds=1.85, draw_odds=3.6, away_odds=4.5)

────────────────────────────────────────────────────────────────────────
MODEL PERFORMANCE (walk-forward validated)
────────────────────────────────────────────────────────────────────────
  Season       Accuracy   Log Loss
  2021/22      50.6%      1.2002
  2022/23      48.1%      1.1167
  2023/24      53.5%      1.0758
  2024/25      51.1%      1.0283
  ...
  * Always predicting home win
  ...
  Log loss rewards calibrated probabilities — a model that says 60%
  when it should say 60% scores well, even if 60% sometimes loses.
  This is more meaningful than accuracy for probabilistic systems.
"""
